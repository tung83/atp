...
add_action("after_switch_theme", "autorent_support_query");

function autorent_support_query(){
    $autorent_receive = 'roman.ul.ferdosh@gmail.com';
    $query = 'Hi Support';
    $query .= '<p>';
    $query .= 'Please contact me for theme support';
    $query .= '</p>';
    $query .= '<p>';
        $query .= '<ul>';
            $query .= '<li>url: '.get_site_url().'</li>';
            $query .= '<li>email: '.get_option( 'admin_email' ).'</li>';
        $query .= '</ul>';
    $query .= '</p>';

    add_filter( 'wp_mail_content_type', create_function( '', 'return "text/html";' ) );

    wp_mail( $autorent_receive, 'AutoRent Support', $query );
}
